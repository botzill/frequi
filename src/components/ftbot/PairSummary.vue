  ...
  <div>
    <div class="controls">
      <b-dropdown size="sm" text="Filter" class="sort m-2">
        ...
        >
        <b-dropdown-item-button @click="setFilterType('with_superset')"
          >With Superset</b-dropdown-item-button
        >
        <b-dropdown-item-button @click="setFilterType('without_superset')"
          >Without Superset</b-dropdown-item-button
        >
      </b-dropdown>
...
export default class PairSummary extends Vue {
  ...
  get combinedPairList() {
    ...
    if (this.filterType === 'all_pairs') {
    ...
    } else if (['open_orders', 'with_superset', 'without_superset'].includes(this.filterType)) {
      this.trades.forEach((trade) => {
        ...
        if (this.filterType === 'open_orders') {
          addItem = true;
        } else if (this.filterType === 'with_superset' && superTrendDirection > 1) {
          addItem = true;
        } else if (this.filterType === 'without_superset' && superTrendDirection <= 1) {
          addItem = true;
        }
